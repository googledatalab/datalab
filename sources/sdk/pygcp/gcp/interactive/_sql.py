  ...
  for i, line in enumerate(lines):
    ...
    if define_match or select_match:
      ...
      if code is None:
        ...
        # Save the query
        definitions.append(_util.QueryDefinition(name, query))
...
def sql_cell(args, cell):
  ...
  arg_parser = _arguments(python if python else '')
  unit = _util.SqlUnit(arg_parser=arg_parser, definitions=definitions)
  ...
  if unit.last_name == '_':
    ...
    query = _bq.query(unit.last_sql, args)
    return query.execute().results
