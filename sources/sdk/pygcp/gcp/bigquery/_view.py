...
class View(_BaseTable):
  ...
  exposes a number of the same APIs as Table and Query through wrapper functions around these.
  """
  ...
  def query(self):
    """The View query."""
    return self._table.metadata.view_query if self.exists() else None
  ...
  def create(self, query, overwrite=False):
  ...
  def update(self, friendly_name=None, description=None, query=None):
    ...
    Returns:
    """
    self._table.update(friendly_name=friendly_name, description=description, query=query)
  ...
  def __str__(self):
    """Returns a string representation of the view using its specified name.
