...
class Table(object):
  """Represents a Table object referencing a BigQuery table or view.

     This object can be used to inspect tables and create SQL queries against tables and views.
  """
  ...
  def _create(self, schema=None, query=None, overwrite=False):
    """ Create the table with the specified schema (if Table) or query (if View).
        Private because this should not be exposed; it is called from the public create
        methods for Table and View.
  ...
  def create(self, schema, overwrite=False):
    ...
    """
    return self._create(schema=schema, overwrite=overwrite)
