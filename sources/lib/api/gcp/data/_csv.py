...
class Csv(object):
  ...
  def _read_gcs_lines(self, max_lines=None):
    return gcp.storage.Item.from_url(self.path).read_lines(max_lines)

  def _read_local_lines(self, max_lines=None):
    lines = []
    for line in open(self.path).xreadlines():
  ...
  def _infertype(self, values, max_labels):
    if all(self._isfloat(e) for e in values):
      return 'numeric', None
  ...
  def browse(self, max_lines, headers):
    ...
    if self.path.startswith('gs://'):
      lines = self._read_gcs_lines(max_lines)
    else:
      lines = self._read_local_lines(max_lines)
