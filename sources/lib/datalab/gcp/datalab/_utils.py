...
def replace_vars(config, env):
  ...
  Args:
    v: the config to parse. Can be a tuple, list or dict.
    env: user supplied dictionary.
...
def validate_gcs_path(path, require_object):
  ...
  """
  bucket, key = gcp.storage._bucket.parse_name(path)
  if bucket is None:
...
def chart_html(driver_name, chart_type, source, chart_options=None, fields='*', refresh_interval=0,
  ...
  if chart_options is not None and 'variables' in chart_options:
    ...
    try:
      ...
      _, variable_defaults = gcp.data.SqlModule.get_sql_statement_with_environment(item, '')
    except Exception:
      variable_defaults = {}
  ...
  _HTML_TEMPLATE = """
    ...
    <script>
      require(['extensions/charting', 'element!{id}', 'base/js/events',
  ...
  return _HTML_TEMPLATE \
      .format(driver=driver_name,
              ...
              refresh_data=json.dumps(refresh_data, cls=gcp._util.JSONEncoder),
