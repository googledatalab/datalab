...
// Wrappers for Plotly.js and Google Charts
  ...
  abstract class ChartLibraryDriver {
    ...
    abstract requires(chartStyle:string):Array<string>;
  ...
  class PlotlyDriver extends ChartLibraryDriver {
    ...
    requires(chartStyle:string):Array<string> {
      return ['d3', 'plotly'];
    }
      ...
      if (this.chartStyle == 'line' || this.chartStyle == 'scatter') {
        ...
        for (var i = 0; i < data.rows.length; i++) {
          ...
          for (var j = 0; j < data.cols.length; j++) {
            row[j] = entry[j].v;
            hoverrow[j] = hoverx + '= ' + xlabels[j] + ', ' + hovery + '= ' + ylabels[i] + ': ' + row[j];
          }
          points[i] = row;
  ...
  class GChartsDriver extends ChartLibraryDriver {
    ...
    requires(chartStyle:string):Array<string> {
      ...
      return ['visualization!' + chartScript];
    }
  ...
  class Chart {
    ...
    // Get a string representation of the current environment - i.e. control settings and refresh data.
    ...
    protected removeStaticChart():void {
      var cell = this.getCell();
      if (cell) {
        var pngDivs = <NodeListOf<HTMLDivElement>>cell.element[0].getElementsByClassName('output_png');
        if (pngDivs) {
            ...
            if (chartDivs) {
              for (var j = 0; j < chartDivs.length; j++) {
                ...
                if (chartDivs[j].id == this.dom.id) {
                  ...
                  var pngDivs = <NodeListOf<HTMLDivElement>>outputDiv.getElementsByClassName('output_png');
                  if (pngDivs) {
    ...
    // Add a static chart (PNG) to the notebook. The notebook will in turn add it to the DOM when the notebook
    // is opened.
  ...
  function _render(driver:ChartLibraryDriver,
    ...
    require(driver.requires(chartStyle), function (/* ... */) {
